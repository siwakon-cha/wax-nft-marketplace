...
const AssetImage = (props) => {
    ...
    useEffect(() => {
        ...
        if (imageFormats[imagePosition] === 'video') {
            setVideoPlayer(
                <video width="400" height="400" loop controls autoPlay={true} muted={true} poster={image ? image : backimg}>
    ...
    return (
        <div className="AssetImage">
            ...
            : imageFormats[imagePosition] === 'model' && process.browser && modelViewer ?
                modelViewer : (
                <img src={images[imagePosition]} alt="none" onClick={imageFormats[imagePosition] !== 'image' ? () => {} : toggleImage} />
            )}
            <div className="ImageButtons">
                {
                    images.map((image, index) =>
                        images.length > 1 ? (<div className="AssetNextButtons" onClick={
