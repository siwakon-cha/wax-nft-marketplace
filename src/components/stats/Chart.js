...
const Chart = (props) => {
    ...
    const GetRevenueHistory = async(range, author) => {
        ...
        setChartRange(range);
        Promise.all([
            get('volume-graph/' + range + '/' + (author ? author : '*'), 'statistics').then((result) => {setRevenueHistory(result)}),
            get('num-graph/' + range + '/' + (author ? author : '*'), 'statistics').then((result) => {setAssetHistory(result)}),
        ]);
    ...
    if (isLoading || !revenueHistory || !revenueHistory['revenueHistory'] || !assetHistory || !assetHistory['assetHistory'])
        return (
    ...
    const items = {
        ...
        'assetHistory': assetHistory['assetHistory']
    };
    ...
    const chartOptions = {
        ...
        series: [
            ...
            },
            {
                type: 'column',
                borderWidth: 0,
                name: t('analytics.assets_created'),
                color: '#FEFEFE',
                data: items['assetHistory'].map(item => [item['date'], item['assetsCreated'] ? item['assetsCreated'] : 0] ),
                yAxis: 1,
                tooltip: {
                    valueSuffix: '',
                    valuePrefix: ''
