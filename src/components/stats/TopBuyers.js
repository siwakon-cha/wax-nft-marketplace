...
function TopBuyers(props) {
    ...
    const GetRevenueHistory = async(range, author) => {
        ...
        const result = await get(`buyer-volume-table/${range}/${(author ? author : '*')}`, 'statistics').then(getChartResult);
    ...
    const chartDropDownOptions = [
        { value: 1, label: t('analytics.24h_top_buyers') },
        { value: 2, label: t('analytics.days_top_buyers', {days: 2}) },
        { value: 3, label: t('analytics.days_top_buyers', {days: 3}) },
        { value: 7, label: t('analytics.days_top_buyers', {days: 7}) },
        { value: 15, label: t('analytics.days_top_buyers', {days: 15}) },
        { value: 30, label: t('analytics.days_top_buyers', {days: 30}) },
        { value: 60, label: t('analytics.days_top_buyers', {days: 60}) },
        { value: 90, label: t('analytics.days_top_buyers', {days: 90}) },
        { value: 120, label: t('analytics.days_top_buyers', {days: 120}) },
        { value: 180, label: t('analytics.days_top_buyers', {days: 180}) },
        { value: 270, label: t('analytics.days_top_buyers', {days: 270}) },
        { value: 365, label: t('analytics.days_top_buyers', {days: 365}) },
    ];
            ...
            <div className="TopMarketsTable">
                <table cellSpacing="0">
                    <tr>
                        ...
                        <th onClick={() => selectSortBy('amount')}>{t('analytics.buys')}</th>
                    ...
                    <LoadingIndicator /> :
                        revenueHistory.sort((a, b) => (['volume', 'usdVolume', 'amount'].includes(sortBy) ? (b[sortBy] - a[sortBy]) : ('' + b[sortBy]).localeCompare(a[sortBy])) * sortDir).slice(0, showNum).map((ranking, index) => {
                            return (
                                <tr className={ual['activeUser'] && ranking['buyer'] === ual['activeUser']['accountName'] ? "TopMarketsInfo HighlightUser" : "TopMarketsInfo" } >
                                    <td>{index + 1}</td>
                                    <td><Link className={"NextLink"} href={`/user/${ranking['buyer']}`}>{ ranking['buyer'] }</Link></td>
